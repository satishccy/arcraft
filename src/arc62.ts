...
import { getAlgodClient, getIndexerClient, base64ToUint8Array } from './utils';
import { READ_ACCOUNT, simulateRequest } from './const';
import { arc62Abi } from './abis';
import { Arc54 } from './arc54';
...
export class Arc62 {
   ...
   * ```
   */
  static async isArc62Compatible(
    assetId: number,
    network: Network
  ): Promise<{
    compatible: boolean;
    ...
    const asset = await AssetFactory.fromId(assetId, network);
      ...
      if ('properties' in metadata) {
        if ('arc-62' in metadata.properties) {
          if ('application-id' in metadata.properties['arc-62']) {
            const applicationId =
              metadata.properties['arc-62']['application-id'];
            try {
    ...
    try {
      ...
      if (latest && latest.note) {
        ...
        try {
          const noteBytes =
            note instanceof Uint8Array ? note : base64ToUint8Array(note);
          ...
          if (arc62.length > 1 && arc62[1][0] === 'j') {
            ...
            const decoded = algosdk.msgpackRawDecode(msgpack) as any;
  ...
  static async getCirculatingSupply(
    ...
    if (isArc62Compatible.compatible) {
      ...
      atc.addMethodCall({
        ...
        method: arc62Abi.getMethodByName('arc62_get_circulating_supply'),
        methodArgs: [assetId],
        sender: READ_ACCOUNT,
        suggestedParams,
      ...
      const returnValue = simulateResult.methodResults[0].returnValue as bigint;
