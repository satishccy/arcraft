...
export class Arc82 {
  ...
  private static readonly SCHEME = 'algorand';
  /** Path prefix for application URIs */
  private static readonly APP_PATH_PREFIX = '//app/';
  /** Path prefix for asset URIs */
  private static readonly ASSET_PATH_PREFIX = '//asset/';
  ...
  static parse(uri: string): ParsedAlgorandUri {
    ...
    if (url.protocol !== `${this.SCHEME}:`) {
      throw new Arc82ParseError(
        `Invalid scheme. Expected '${this.SCHEME}:'`,
        uri
      );
    }

    // Determine type and extract ID
    let type: AlgorandUriType;
    let id: number;
    ...
    if (uri.includes(this.APP_PATH_PREFIX)) {
      ...
      throw new Arc82ParseError(
        'URI must contain either //app/ or //asset/ path',
        uri
      );
    }
    ...
    const result: ParsedAlgorandUri = {
      type,
      id,
      originalUri: uri,
    };
  ...
  private static parseAppParams(
    ...
    if (boxParams.length > 0) {
      ...
      if (params.box.length !== boxParams.length) {
        throw new Arc82ParseError(
          'Invalid base64url encoding in box parameter',
          uri
        );
      }
    }
    ...
    if (globalParams.length > 0) {
      params.global = globalParams.filter((param) =>
        this.isValidBase64Url(param)
      );
      if (params.global.length !== globalParams.length) {
        throw new Arc82ParseError(
          'Invalid base64url encoding in global parameter',
          uri
        );
      }
    }
    ...
    if (localParams.length > 0) {
      ...
      for (const localParam of localParams) {
        if (!this.isValidBase64Url(localParam)) {
          throw new Arc82ParseError(
            'Invalid base64url encoding in local parameter',
            uri
          );
        }
        ...
        if (addresses.length === 0) {
          throw new Arc82ParseError(
            'Local storage queries require algorandaddress parameter',
            uri
          );
        }
  ...
  static buildAppUri(id: number, params?: AppQueryParams): string {
    if (id < 0 || !Number.isInteger(id)) {
      throw new Arc82ParseError(
        'Application ID must be a non-negative integer'
      );
    }
      ...
      if (params.box) {
        for (const boxKey of params.box) {
          if (!this.isValidBase64Url(boxKey)) {
            throw new Arc82ParseError(
              'Box keys must be valid base64url encoded'
            );
          }
      ...
      if (params.global) {
        for (const globalKey of params.global) {
          if (!this.isValidBase64Url(globalKey)) {
            throw new Arc82ParseError(
              'Global keys must be valid base64url encoded'
            );
          }
      ...
      if (params.local) {
        for (const localParam of params.local) {
          if (!this.isValidBase64Url(localParam.key)) {
            throw new Arc82ParseError(
              'Local keys must be valid base64url encoded'
            );
          }
  ...
  static buildAssetUri(id: number, params?: AssetQueryParams): string {
    ...
    if (params) {
      ...
      for (const [key, value] of Object.entries(params)) {
        if (
          value === true &&
          Object.values(AssetParamType).includes(key as AssetParamType)
        ) {
      ...
      if (parsedUri.appParams) {
        ...
        if (parsedUri.appParams.local) {
          ...
          for (const localParam of parsedUri.appParams.local) {
            ...
            const appsLocalState = accountLocalState.filter(
              (state: ApplicationLocalState) =>
                state.id === BigInt(parsedUri.id)
            );
            const appLocalState =
              appsLocalState.length > 0 ? appsLocalState[0] : undefined;
            let localState: LocalStorageData | undefined;
   ...
   * ```
   */
  static async queryFromUri(
    uri: string,
    network: Network
  ): Promise<AppQueryResult | AssetQueryResult> {
    const parsed = this.parse(uri);
...
export class Arc82Utils {
   ...
   * ```
   */
  static validateGrammar(uri: string): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
        ...
        if (parsed.appParams.local && parsed.appParams.local.length > 0) {
          for (const localParam of parsed.appParams.local) {
            if (!localParam.algorandaddress) {
              errors.push(
                'Local storage query requires algorandaddress parameter'
              );
            }
          }
        }
