...
export class Arc19 extends CoreAsset {
  ...
  static getMetadataVersions = async (assetId: number, network: Network) => {
    ...
    for (var i = 0; i < assets_txns.transactions.length; i++) {
      if (i == 0) {
        url =
          (assets_txns.transactions[i] as any)["asset-config-transaction"]?.params?.url || 
          assets_txns.transactions[i].assetConfigTransaction?.params?.url || "";
      }
      var round = (assets_txns.transactions[i] as any)["confirmed-round"] || 
                  assets_txns.transactions[i].confirmedRound || 0;
      var metadata_url = this.resolveProtocol(
        url,
        (assets_txns.transactions[i] as any)["asset-config-transaction"]?.params?.reserve ||
