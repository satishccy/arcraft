...
class Arc59 {
    ...
    static async getAssetSendInfo(network, assetId, receiver) {
        ...
        atc.addMethodCall({
            appID: appInfo.appId,
            method: abis_1.arc59Abi.getMethodByName('arc59_getSendAssetInfo'),
    ...
    static async getInboxAddress(network, receiver) {
        ...
        atc.addMethodCall({
            appID: appInfo.appId,
            method: abis_1.arc59Abi.getMethodByName('arc59_getInbox'),
            methodArgs: [receiver],
            sender: const_1.READ_ACCOUNT,
    ...
    static async sendAsset({ network, assetId, amount, receiver, sender, }) {
        ...
        sendInfo.receiverAlgoNeededForClaim =
            sendInfo.receiverAlgoNeededForClaim !== 0n
                ...
                : 0n;
        const composer = new algosdk_1.AtomicTransactionComposer();
        if (sendInfo.mbr > 0n || sendInfo.receiverAlgoNeededForClaim > 0n) {
            const mbrPayment = algosdk_1.default.makePaymentTxnWithSuggestedParamsFromObject({
                sender: sender.address,
                suggestedParams,
        ...
        if (!sendInfo.routerOptedIn) {
            composer.addMethodCall({
                appID: appInfo.appId,
                method: abis_1.arc59Abi.getMethodByName('arc59_optRouterIn'),
                methodArgs: [assetId],
                sender: sender.address,
                suggestedParams,
                signer: sender.signer,
        ...
        const boxes = [algosdk_1.default.decodeAddress(receiver).publicKey];
        const inboxAddress = await this.getInboxAddress(network, receiver);
        const axfer = algosdk_1.default.makeAssetTransferTxnWithSuggestedParamsFromObject({
            sender: sender.address,
            suggestedParams,
            assetIndex: assetId,
            receiver: appInfo.appAddress,
        ...
        composer.addMethodCall({
            appID: appInfo.appId,
            method: abis_1.arc59Abi.getMethodByName('arc59_sendAsset'),
            methodArgs: [
                axferWithSigner,
                receiver,
                sendInfo.receiverAlgoNeededForClaim,
    ...
    static async getAssetsInInbox({ network, receiver, }) {
        ...
        const appInfo = this.getAppInfo(network);
        const inboxAddress = await this.getInboxAddress(network, receiver);
        if (inboxAddress !== algosdk_1.default.ALGORAND_ZERO_ADDRESS_STRING) {
    ...
    static async claimAsset({ network, receiver, assetId, }) {
        ...
        if (inboxInfo.amount > inboxInfo.minBalance) {
            ...
            composer.addMethodCall({
                appID: appInfo.appId,
                method: abis_1.arc59Abi.getMethodByName('arc59_claimAlgo'),
                methodArgs: [],
        ...
        composer.addMethodCall({
            appID: appInfo.appId,
            method: abis_1.arc59Abi.getMethodByName('arc59_claim'),
            methodArgs: [assetId],
            sender: receiver.address,
            suggestedParams: {
                ...suggestedParams,
            ...
            boxes: [
                {
                    appIndex: appInfo.appId,
                    name: algosdk_1.default.decodeAddress(receiver.address).publicKey,
                },
            ],
    ...
    static async rejectAsset({ network, receiver, assetId, }) {
        ...
        composer.addMethodCall({
            appID: appInfo.appId,
            method: abis_1.arc59Abi.getMethodByName('arc59_reject'),
            methodArgs: [assetId],
            sender: receiver.address,
            suggestedParams: {
                ...suggestedParams,
