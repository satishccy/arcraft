...
export const detectIdType = (
  ...
  if (keyLower === 'id' && typeof value === 'number' && value > 0)
    return 'asset';
  ...
  if (
    keyLower.includes('txn') &&
    typeof value === 'string' &&
    value.length === 52
  )
    return 'tx';
  if (keyLower === 'txid' && typeof value === 'string' && value.length === 52)
    return 'tx';
      ...
      <span>
        ...
        <div style={{ marginLeft: '1rem' }}>
          {data.map((item, index) => (
            <div key={index}>
              {indentStr}{' '}
              <JsonRenderer data={item} network={network} indent={indent + 1} />
              {index < data.length - 1 && ','}
            </div>
          ))}
        </div>
